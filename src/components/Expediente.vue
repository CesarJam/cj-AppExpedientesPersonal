...
// 1. LISTA DE CATEGORÍAS (Basada en requisitos de Nuevo Ingreso)
const CATEGORIAS = [
...
<template>
    <div class="p-6 max-w-7xl mx-auto transition-colors duration-300">
        ...
        <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm mb-6 border border-gray-200 dark:border-gray-700 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 transition-colors duration-300">
            <div class="flex items-center gap-4">
                <img v-if="empleado.foto_url" :src="empleado.foto_url" class="h-16 w-16 rounded-full object-cover shadow-sm border border-gray-200 dark:border-gray-600" />
                <div v-else class="h-16 w-16 rounded-full bg-blue-100 dark:bg-blue-900/50 text-blue-600 dark:text-blue-400 flex items-center justify-center font-bold text-2xl shadow-sm border border-blue-200 dark:border-blue-800">
                    {{ empleado.nombre_completo.charAt(0) }}
                </div>
                
                <div>
                    <h2 class="text-2xl font-bold text-gray-800 dark:text-white">{{ empleado.nombre_completo }}</h2>
                    <p class="text-sm text-gray-500 dark:text-gray-400">RFC: <span class="font-mono text-gray-700 dark:text-gray-300">{{ empleado.rfc }}</span></p>
                </div>
            </div>

            <span class="bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 px-4 py-1.5 rounded-full text-sm font-bold border border-blue-100 dark:border-blue-800 shadow-sm">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                            ...
                            <div v-for="doc in docs" :key="doc.id" class="p-4 flex items-center justify-between hover:bg-gray-50 dark:hover:bg-gray-700/50 transition group">
                                ...
                                <div class="flex items-center gap-3 shrink-0">
                                    ...
                                    <template v-if="docConfirmando !== doc.id">
                                        <button @click="abrirVisorPDF(doc.url_storage)" class="text-xs font-bold text-blue-600 dark:text-blue-400 hover:underline uppercase transition">Abrir PDF</button>
            ...
            <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 h-fit sticky top-24 transition-colors duration-300">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <select v-model="categoriaSeleccionada"
                            class="w-full bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none transition-colors">
                    ...
                    <button @click="subirDocumento" :disabled="!archivoSeleccionado || subiendo"
                        class="w-full bg-blue-600 dark:bg-blue-700 text-white font-bold py-2 rounded-lg hover:bg-blue-700 dark:hover:bg-blue-600 transition disabled:opacity-50">
                        {{ subiendo ? 'Guardando...' : 'Subir al Expediente' }}
                    </button>

                    <div 
                        v-if="mensajeExito" 
                        class="mt-4 p-3 bg-green-50 dark:bg-green-900/30 border border-green-200 dark:border-green-800 text-green-700 dark:text-green-400 rounded-lg flex items-center gap-2 text-sm font-medium animate-fade-in"
                    >
        ...
        <div v-if="pdfSeleccionado" class="fixed inset-0 bg-black/80 flex flex-col z-[100] backdrop-blur-sm animate-fade-in">
            ...
            <div class="flex-1 w-full p-2 md:p-6 pt-2">
                <iframe 
                    :src="pdfSeleccionado" 
                    class="w-full h-full rounded-xl bg-white shadow-2xl border-0"
                    title="Visor PDF"
                ></iframe>
            </div>
        </div>
